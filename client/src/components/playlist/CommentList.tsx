...
const CommentList: FC<SocketProps> = ({ initialValue, setInitialValue }) => {
  ...
  useEffect(() => {
    const handleNewComment = (comments: any) => {
      ...
      if (setInitialValue) setInitialValue(updatedData);
    };
  ...
  const renderComments = ({ item }: any) => {
    return (
      <Animated.View
        ...
        style={[commonStyles.flexRowBetween, interactionStyles.item]}
      >
        ...
        ListEmptyComponent={
          <View style={interactionStyles.emptyContainer}>
            <CustomText style={interactionStyles.center}>
              There are no comments yet! Be fearless and comment
            </CustomText>
          </View>
        }
        windowSize={4}
